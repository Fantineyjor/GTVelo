...
from gtvelo.models.velocity_field import ATACRegVelocityField
from gtvelo.models.modules import MLP,TransImg
...
class ATACRegModel(nn.Module):
    ...
    def __init__(self, observed, latent_dim = 20, zr_dim = 2, h_dim = 2,
        ...
        if batch_correction:
            ...
            self.encoder_z0_s = TransImg(observed,  2*latent_dim,encoder_hidden)
            self.encoder_z0_u = TransImg(observed, 2*latent_dim,encoder_hidden, )
            self.encoder_z0_atac = TransImg(observed,2*latent_dim, encoder_hidden )
        ...
        self.initial = nn.Linear(1, self.h_dim//2) 
    ...
    def _run_dynamics(self, c, times, test=False):
        ...
        h0 = th.cat((h0, th.zeros(c.shape[0], self.h_dim//2).cuda(), c), dim=-1)#60+2+60
        
        if test:
            ht_full = odeint(self.velocity_field, h0, th.cat((th.zeros(1).cuda(), times), dim=-1), method='dopri8', options=dict(max_num_steps=1000)).permute(1,0,2) #
        else: 
    ...
    def loss(self, normed_s, s, s_size_factor, mask_s, normed_u, u, u_size_factor, mask_u, normed_a, velo_genes_mask, adj, root_cells, batch_id = None, epoch = None):
        ...
        latent_state, zs, zu, latent_time, time_mean, time_logvar = self.latent_embedding(normed_s, normed_u, normed_a, adj, batch_id = batch_id)
        ...
        zs = zs[sort_index][index]
        zu = zu[sort_index][index]
        ...
        if self.likelihood_model == 'gaussian':
            shat_data = shat_data.detach()
            uhat_data = uhat_data.detach()
            shat = shat.detach()
            uhat = uhat.detach()
            ...
            MSE_A= th.sum(mask_a*(ahat_data - normed_a)**2, dim=-1) + th.sum(mask_a*(ahat - normed_a)**2, dim=-1)
            MSE_S_U=th.sum(mask_s*(shat_data - s)**2, dim=-1) + th.sum(mask_u*(uhat_data - u)**2, dim=-1)+th.sum(mask_s*(shat - s)**2, dim=-1) + th.sum(mask_u*(uhat - u)**2, dim=-1)
        ...
        #return reconstruction_loss, validation_ae.detach(), validation_traj.detach(), corr_reg_val.unsqueeze(-1).detach(), orig_index    
        return reconstruction_loss, MSE_S_U.detach(), MSE_A.detach(), corr_reg_val.unsqueeze(-1).detach(), orig_index    
    ...
    def latent_embedding(self, normed_s, normed_u, normed_a, adj, batch_id = None):
        ...
        zs = self.encoder_z0_s(normed_s,adj)
        zu = self.encoder_z0_u(normed_u,adj)    
        za = self.encoder_z0_atac(normed_a,adj)
