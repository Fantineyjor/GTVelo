...
def standard_clean_recipe(adata, spliced_key = 'spliced', unspliced_key = 'unspliced', batch_key = None, root_cells = None, terminal_cells = None,
    ...
    adata.X = scp.sparse.csr_matrix(adata.layers[spliced_key].copy())#将数据存储为稀疏矩阵格式，减少内存
    ...
    # include raw counts
    adata.layers['spliced_counts'] = np.array(adata.layers[spliced_key])#原始数据存在这里面
...
def atac_clean_recipe(adata, spliced_key = 'spliced', unspliced_key = 'unspliced', batch_key = None, root_cells=None, terminal_cells=None, celltype_key='celltype',
    ...
    """
    Clean and setup data for ATAC+RNA version of gtvelo
    """
    if scp.sparse.issparse(adata.layers[spliced_key]):
        adata.layers[spliced_key] = np.array(adata.layers[spliced_key].todense())
        adata.layers[unspliced_key] = np.array(adata.layers[unspliced_key].todense())
    else:
        adata.layers[spliced_key] = np.array(scp.sparse.csr_matrix(adata.layers[spliced_key]).todense())
        adata.layers[unspliced_key] = np.array(scp.sparse.csr_matrix(adata.layers[unspliced_key]).todense())

    adata.layers['spliced_counts'] = np.array(adata.layers[spliced_key])
    adata.layers['unspliced_counts'] = np.array(adata.layers[unspliced_key])
    adata.layers['atac'] = np.array(adata.layers['Mc'].todense())
   
    if log:
        scv.pp.log1p(adata)
    if normalize_library:

        spliced_data = np.array(adata.layers[spliced_key])
        unspliced_data = np.array(adata.layers[unspliced_key])

        spliced_library_sizes = spliced_data.sum(axis=1, keepdims=True)
        unspliced_library_sizes = unspliced_data.sum(axis=1, keepdims=True)

        spliced_median_library_size = np.median(spliced_library_sizes)
        unspliced_median_library_size = np.median(unspliced_library_sizes)

        if spliced_median_library_size == 0:
            raise ValueError("Spliced median library size is 0")
        if unspliced_median_library_size == 0:
            raise ValueError("Unspliced median library size is 0")

        spliced_size_factors = spliced_library_sizes / spliced_median_library_size
        unspliced_size_factors = unspliced_library_sizes / unspliced_median_library_size

        spliced_size_factors[spliced_size_factors == 0] = 1
        unspliced_size_factors[unspliced_size_factors == 0] = 1

        adata.layers[spliced_key] = spliced_data / spliced_size_factors
        adata.layers[unspliced_key] = unspliced_data / unspliced_size_factors

        adata.obs['spliced_size_factor'] = spliced_size_factors
        adata.obs['unspliced_size_factor'] = unspliced_size_factors

        print("\nAfter normalization check:")
        print(f"Any NaN in normalized spliced: {np.any(np.isnan(adata.layers[spliced_key]))}")
        print(f"Any NaN in normalized unspliced: {np.any(np.isnan(adata.layers[unspliced_key]))}")
    ...
    adata.layers['mask_spliced'] = (adata.layers[spliced_key] > 0).astype(int)
    adata.layers['mask_unspliced'] = (adata.layers[unspliced_key] > 0).astype(int)
    
    if log:
        scv.pp.log1p(adata)

    print("\nFinal check:")
    print(f"Any NaN in final spliced: {np.any(np.isnan(adata.layers[spliced_key]))}")
    print(f"Any NaN in final unspliced: {np.any(np.isnan(adata.layers[unspliced_key]))}")
