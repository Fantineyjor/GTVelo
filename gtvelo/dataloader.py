...
class Dataset(th.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return_dict = {
            'S': th.Tensor(self.adata[i:j].layers['spliced_counts'].astype(float)),
        ...
        # 如果 'batch_onehot' 存在于 obsm 中，则添加到字典中
        if 'batch_onehot' in self.adata[i:j].obsm:
            return_dict['batch_onehot'] = th.Tensor(self.adata[i:j].obsm['batch_onehot']).long()
