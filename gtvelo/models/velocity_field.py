...
from gtvelo.models.modules import MLP
...
class VelocityFieldReg(nn.Module):
    ...
    def __init__(self, latent, h_dim, zr_dim, include_time = False, linear_splicing=True):
        ...
        if include_time:
            self.unspliced_net = MLP(latent+zr_dim+1, [latent+5], latent).to(device)
        else:
            self.unspliced_net = MLP(latent+zr_dim, [latent+5], latent).to(device)

        if linear_splicing:
            self.spliced_net = nn.Linear(2*latent, latent).to(device)
        else:
            self.spliced_net = MLP(2*latent, [latent+5], latent).to(device)
        
        self.reg_net = MLP(latent + zr_dim + h_dim, [latent+5], zr_dim).to(device)
        ...
        for m in self.reg_net.modules():#f_r
            if isinstance(m, nn.Linear):
                nn.init.normal_(m.weight, mean=0, std=0.01)
                nn.init.constant_(m.bias, val=0.0)
    ...
    def forward(self, t, z):
        ...
        if self.include_time:#时间变量，如果 include_time 为 True，则会被包括在输入中。
